...
build: check-path get _go_test _megdcui
...
_megdcui:
	rm -f megdcui
	go build $(GO_EXTRAFLAGS)  -ldflags="-X main.date=$(shell date +%Y-%m-%d_%H:%M:%S%Z)"  -o megdcui ./cmd/megdcui


_megui:
	./megdcui -v start --config $(MEGAM_HOME)/megdc/megdcui.conf
...
deadcode: _install_deadcode
	@go list ./... | sed -e 's;github.com/megamsys/megdcui/;;' | xargs deadcode
